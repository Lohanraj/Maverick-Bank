...
namespace MaverickBank.API.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        // UPDATE OWN PROFILE (ALL ROLES — can only update personal fields, not role)
        [Authorize]
        [HttpPut("profile")]
        public IActionResult UpdateProfile(UpdateProfileRequest request)
        {
            ...
            int userId = int.Parse(userIdClaim.Value);
            var user = _userService.UpdateProfile(userId, request, out string errorMessage);

            if (user == null)
            {
                if (errorMessage == "User not found") return NotFound(errorMessage);
                return BadRequest(errorMessage);
            }
        ...
        {
            var success = _userService.DeleteUser(id, out string errorMessage);
            if (!success)
            {
                if (errorMessage == "User not found")
                    return NotFound(errorMessage);
            ...
            _logger.LogInformation("User {UserId} was permanently deleted by admin", id);

            return Ok(new { Message = "User account and all related records deleted successfully." });
        }
    }
}
